function [t,mh1Matrix,mh7Matrix]=dCmodelNPF(parameters,pb1,pb2)
% Function to simulate the No Positive Feedback model with given parameters and perturbations
% Inputs:
...
for ti=1:T-1
    ...
    for k=1:L
        ...
        % Euler's method
        for n=1:N
            % Update deltaC mRNA (No Positive Feedback)
            if ti>tmdC
                den=1+(ph11Matrix(k,ti-tmdC)/critph11)^2+(ph67Matrix(k,ti-tmdC)/critph67)^2;
                mdC(n+1)=mdC(n)+h*(maxmdC_syn*(1/den*E1)+mdC_deg*mdC(n));
            else
                mdC(n+1)=mdC(n);
            end
