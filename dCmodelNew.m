function [t,mh1Matrix,mh7Matrix]=dCmodelNew(parameters,pb1,pb2)
...
for ti=1:T-1
    ...
    for k=1:L
        ...
        tmdD = round(parameters(65)); % deltaD mRNA transcription time-delay
        ...
        for n=1:N
            ...
            % Update deltaD mRNA (With Positive Feedback)
            if ti>tmdD
                if k==1
                    den=1+(pdCCMatrix(2,ti-tmdD)/critpdCC)+(pdCDMatrix(2,ti-tmdD)/critpdCD)+(pdDDMatrix(2,ti-tmdD)/critpdDD);
                    mdD(n+1)=mdD(n)+h*(mdD_syn*(1/den*E1+(pdCCMatrix(2,ti-tmdD)/critpdCC)/den*ECC+(pdCDMatrix(2,ti-tmdD)/critpdCD)/den*ECD+(pdDDMatrix(2,ti-tmdD)/critpdDD)/den*EDD)-mdD_deg*mdD(n));
                else
                    den=1+(pdCCMatrix(1,ti-tmdD)/critpdCC)+(pdCDMatrix(1,ti-tmdD)/critpdCD)+(pdDDMatrix(1,ti-tmdD)/critpdDD);
                    mdD(n+1)=mdD(n)+h*(mdD_syn*(1/den*E1+(pdCCMatrix(1,ti-tmdD)/critpdCC)/den*ECC+(pdCDMatrix(1,ti-tmdD)/critpdCD)/den*ECD+(pdDDMatrix(1,ti-tmdD)/critpdDD)/den*EDD)-mdD_deg*mdD(n));
                end
            else
                mdD(n+1)=mdD(n);
            end
